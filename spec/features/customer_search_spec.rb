...
feature "Customer Search" do
  ...
  scenario "Search by Name" do
    within 'section.search_form' do
      fill_in 'keywords', with: 'pat'
    end
    within 'section.search_results' do
  ...
  scenario "Search by Email" do
    ...
    within 'section.search-results' do
      expect(page).to have_content('Results')
      expect(page.all('lo li.list-group-item')[0]).to have_content('Pat')
      expect(page.all('lo li.list-group-item')[0]).to have_content('Jones')
      
      expect(page.all('lo li.list-group-item')[1]).to have_content('Patricia')
      expect(page.all('lo li.list-group-item')[1]).to have_content('Dobbs')
      
      expect(page.all('lo li.list-group-item')[3]).to have_content('I.T.')
      expect(page.all('lo li.list-group-item')[3]).to have_content('Pat')
